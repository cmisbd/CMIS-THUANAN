...
export class HdgVitriDdoStore {
  ...
  async putMany(list: any[]): Promise<void> {
    ...
    const store = tx.objectStore(GCS_STORE_NAMES.HDG_VITRI_DDO);
...
async bulkSave(items: any[]): Promise<void> {
  ...
  async getAll(): Promise<any[]> {
    const db = await openGcsDb();
    return db.getAll(GCS_STORE_NAMES.HDG_VITRI_DDO);
  }
...
async getByKey(key: { maKhang: string; soCongTo: string }): Promise<any> {
  const db = await openGcsDb();
  return await db.get(GCS_STORE_NAMES.HDG_VITRI_DDO, [
    key.maKhang,
    key.soCongTo
  ]);
}
