...
import { HdgVitriTramStore } from '../indexeddb/hdg-vitri-tram.store';
import { openGcsDb  } from '../indexeddb/db-config';
import { GCS_STORE_NAMES } from '../indexeddb/db-config';
import {HdgVitriTramModel} from '../models/hdg-vitri-tram.model'
...
export class HDGVitriTramRepository {
  private readonly baseUrl = 'https://itthuanan.vn:8081/api/hdg_vitri_tram';
  constructor(private store: HdgVitriTramStore,private http: HttpClient,) {}
  clear() {
  ...
  async findBySoCongTo(soCongTo: string): Promise<HdgVitriTramModel | undefined> {
    const all = await this.store.getAll();
    return all.find(x => x.SO_CONGTO === soCongTo);
}

  /** Đồng bộ toàn bộ bản ghi HDG_VITRI_TRAM trong IndexedDB lên server (set-multi) */
async syncToServer(): Promise<boolean> {
  ...
  try {
    ...
    console.error('Lỗi sync HDG_VITRI_TRAM lên server:', err);
    return false;
  }
}
